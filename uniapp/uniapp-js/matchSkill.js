...
var vm = new Vue({
    ...
    data: {
        ...
        matchPics: [{
                ...
                isShowMask: false,
                remarks: "只针对前挡，照片中需包含（日期码，经纬度、与地点日期）",
    ...
    methods: {
        ...
        btnUploadFile(index) {
            ...
            reader.onloadstart = function(e) {
                _self.matchPics[index].isShowMask = true;
                console.log("开始读取....");
            }
                ...
                if (imgFile.size / 1024 > 1024 * 1.2) {
                    fn.dealImage(evt.target.result, {
                        ...
                        Service.uploadPic('POST', JSON.stringify(params), (function callback(data) {
                            ...
                            if (data.code == 200) {
                                $('.upLoadImgs').eq(index).find(".picFullcar").attr("src", data.data.webPath);
                                _self.matchPics[index].isShowMask = false;
                            }
                        }))
                    });
                } else {
        ...
        add() {
            ...
            for (var i = 0; i < $('.upLoadImgs').length; i++) {
                if ($('.upLoadImgs').eq(i).attr('symType') == 'installProcessImg') {
                    this.installProcessImg = $('.upLoadImgs').eq(i).find(".picFullcar").attr('src')
                } else if ($('.upLoadImgs').eq(i).attr('symType') == 'trademarkImg') {
                    this.trademarkImg = $('.upLoadImgs').eq(i).find(".picFullcar").attr('src')
                } else if ($('.upLoadImgs').eq(i).attr('symType') == 'finishImg') {
                    this.finishImg = $('.upLoadImgs').eq(i).find(".picFullcar").attr('src')
                }
            }

            var params = {
                "finishImg": this.finishImg,
                "installProcessImg": this.installProcessImg,
                "mobile": this.mobile,
                "psersonId": this.personInfo.id,
                "sex": this.sex,
                "trademarkImg": this.trademarkImg,
                "userName": this.userName
            }
            console.log("参数请求：", params)
            Service.joinCompetition('POST', JSON.stringify(params), (function callback(data) {
                //console.log("数据：", data)
                if (data.code == 200) {
                    fn.showTip("提交成功", 'matchSkillList.html');
                }
            }))
