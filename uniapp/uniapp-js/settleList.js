...
var vm = new Vue({
  ...
  methods: {
	...
	_self.params = {
	  ...
	  if (_self.subFlag) {
		...
		var pdat = {
		  pageNo: 1,
		  pageSize: 10,
		  settlementId:this.settlementCompanyId
		}
		Service.getOrderSettleDjsList('POST', JSON.stringify(pdat), (function callback(data) {
		  if (data.code == 200) {
    ...
    toNext() {
      ...
      //2023-2-23 苏州吴中店为苏州福耀门店，同意按苏州福耀的结算规则进行结算
      if (this.shopInfo.id == 1016 || this.shopInfo.id == 1015 || this.shopInfo.id == 2930) {
        if ((new Date().getDate() >= 5 && new Date().getDate() <= 8) || (new Date().getDate() >= 20 && new Date().getDate() <= 23)) {

        } else {
          fn.showTip("结算日期为每月的5号到8号,20号到23号");
          return false;
        }
      } else {
        if (new Date().getDate() < 5 || new Date().getDate() > 8) {
          fn.showTip("结算日期为每月的5号到8号");
          return false;
        }
      }
	  this.params.orderSnList=this.orderSnList

     Service.applyOrderSettle('POST', JSON.stringify(this.params), (function callback(data) {
       if (data.code == 200) {
         fn.showTip('结算申请成功', 'settleListIng.html');
       }
       this.subFlag = true;
    ...
    getListDataFromNet: function (pageNum, pageSize, successCallback, errorCallback) {
      ...
      var params = {
        pageNo: pageNum,
        pageSize: pageSize,
		settlementId:this.settlementCompanyId || '35'
      }
