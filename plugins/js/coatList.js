...
var vm = new Vue({
    ...
    data: {
        ...
        orederType: ["全部", " 待接单", "审核中", "已完成", "审核不通过"],
        arrLevel: [
    ...
    methods: {
        ...
        orderRecevie(e) {
            // if (e.userName) {
            //     window.location.href = "coatDetail.html?orderId=" + e.orderSn;
            // } else{
            //     fn.showTip('暂无客户名称，请先补全', 'editMember.html?kw=' + e.carNo + '&backUrl=coatDetail.html?orderId=' + e.orderSn);
            // }
            window.location.href = "coatDetail.html?orderId=" + e.orderSn;
        ...
        //新建勘验单
        add(){
            window.location.href = "coatQueryMember.html";
        },
        //去详情
        toDetail(e) {
            // if (e.userName) {
            //     window.location.href = "coatDetail.html?orderId=" + e.orderSn;
            // } else{
            //     fn.showTip('暂无客户名称，请先补全', 'editMember.html?kw=' + e.carNo + '&backUrl=coatList.html?orderId=' + e.orderSn);
            // }
            window.location.href = "coatDetail.html?orderId=" + e.orderSn;
        ...
        getListDataFromNet: function(pageNum, pageSize, successCallback, errorCallback) {
            ...
            Service.getPlatingOrderList('POST', JSON.stringify(params), (function callback(data) {
                console.log("=====数据：", data)
                if (data.code == 200) {
                    self.totalPage = data.count;
                    var listData = data.data;
                    for (var i in listData) {
                        listData[i].applyTime = fn.formatTime(listData[i].applyTime,'Y-M-D');
