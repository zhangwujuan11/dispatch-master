...
var vm = new Vue({
  ...
  methods: {
    ...
    //发票信息
    toSettleSend() {
      window.location.href = "settleSend.html";
    },
    //结算详情
    toView(item) {
    ...
    getListDataFromNet: function (pageNum, pageSize, successCallback, errorCallback) {
      ...
      Service.getOrderSettleBdYjslist('POST', JSON.stringify(params), (function callback(data) {
        console.log("=====数据：", data)
        if (data.code == 200) {
    ...
    cngAgree(item) {
      this.pdlist[item].flagSel = !this.pdlist[item].flagSel;
    ...
    surSettle() {
      var _self = this;
      let settlement = []
      this.pdlist.map(item => {
        if (item.flagSel == true) {
          settlement.push(item.settlementSn)
        }
      })
      var params = {
        settlementSn: settlement.join(),
      }
      Service.setExport('POST', JSON.stringify(params), (function callback(data) {
        console.log("=====数据：", data)
        if (data.code == 200) {
