  ...
  methods: {
    getBalance() {
      Service.getShopBalance('GET', '', (function callback(data) {
        console.log(data)
        if (data.code == 200) {
          this.shopBalance = data.data.balance
        }
      }))
    },
    //结算中
    toSettleListIng() {
      window.location.href = "settleBdListIng.html";
    },

    //已结算
    toSettleListEnd() {
      window.location.href = "settleBdListEnd.html";
    },
    ...
    calcTotal() {
      ...
      list.forEach(item => {
        if (item.flagSel === true) {
          ...
          that.orderSnList.push(item.orderSn);
        }
      })
    ...
    getListDataFromNet: function (pageNum, pageSize, successCallback, errorCallback) {
      ...
      //getOrderSettleBdDjsList  getOrderSettleDjsList
      Service.getOrderSettleBdDjsList('POST', JSON.stringify(params), (function callback(data) {
        console.log("=====数据：", data)
        if (data.code == 200) {
    ...
    surSettle() {
      ...
      var params = {
        ...
        // fn.showTip('您的账户余额不足，请先充值', 'settleBdListIng.html');
    
        Service.applyBdOrderSettle('POST', JSON.stringify(params), (function callback(data) {
          console.log("=====数据：", data)
          if (data.code == 200) {
            fn.showTip('确认结算成功，请耐心等待审核！', 'settleBdListIng.html');
          }
          if (data.code == 400) {
            fn.showTip(data.message, 'recharge.html');
          }
          _self.subFlag = true;
        }))
