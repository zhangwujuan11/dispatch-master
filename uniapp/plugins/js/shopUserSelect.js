...
var vm = new Vue({
    ...
    methods: {
        ...
        // //编辑店员信息
        // edit(e) {
        //     window.location.href = "shopUserAdd.html?realId=" + e;
        // },
        //确定选择
        surSel() {
            this.selArrs = [];
            ...
            if (!this.selArrs.length) {
                ...
                var _self = this;
                var activeId = tempJson.activeId ? tempJson.activeId : "";
                var params = {
                    activityId : activeId,
                    personId : selIds
                }
                Service.joinActive('POST', JSON.stringify(params), (function callback(data) {
                    console.log("=====数据：", data)
                    if (data.code == 200) {
                         window.location.href = "activeDetail.html?id=" + activeId;
                    }
                }))
            }
        },
        ...
        getListDataFromNet: function(pageNum, pageSize, successCallback, errorCallback) {
            ...
            //延时一秒,模拟联网
            var params = {
                activityId: tempJson.activeId ? tempJson.activeId : "",
                pageNo: pageNum,
                pageSize: pageSize
            }

            Service.getActivePersonList('POST', JSON.stringify(params), (function callback(data) {
                console.log("=====数据：", data)
                if (data.code == 200) {
