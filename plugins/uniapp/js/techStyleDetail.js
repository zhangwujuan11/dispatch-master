...
var vm = new Vue({
    ...
    methods: {
        ...
        competitorDetail() {
            const that = this;
            let params = {
                competitorId: tempJson.id ? tempJson.id : ""
            }
            Service.getCompetitorDetail('GET', params, (function callback(data) {
                if (data.code == 200) {
                    that.compe = data.data.detail;
                }
            }))
        },
        getDetail() {
            ...
            console.log(this.codePath, 'codepath')
                ...
                if (data.code == 200) {
                    ...
                    setTimeout(function () {
                        var title = _self.info.personName + '邀你来投票！',
                            ...
                            desc = _self.info.introduction,
                            link = window.location.href,
                            imgUrl = _self.info.icon ? _self.info.icon : WebHost + 'images/logo.png';
        ...
        toBack() {
            window.location.href = "techStyle.html"
        },
        getCodeTree() {
            this.number = 0;
        ...
        toVote(e) {
            ...
            $("#codeT3").bind("click", function (ev) {
                ...
                oDiv.style.left = (parseInt(oEvent.pageX) - 15) + 'px'; // 指定创建的DIV在文档中距离左侧的位置    图片大小30 左右移动5
                oDiv.style.top = (parseInt(oEvent.pageY) - 15) + 'px'; // 指定创建的DIV在文档中距离顶部的位置
