...
var vm = new Vue({
  ...
  methods: {
    ...
    toDetail(event) {
      ...
      window.location.href = "claimListDetail.html?orderSn=" + orderSn;
    },
    orderRefuse(item){
      window.location.href = "claimListDetail.html?orderSn=" + item.orderSn;
    },
    ...
    getListDataFromNet: function (pageNum, pageSize, successCallback, errorCallback) {
      ...
      var params = {
        pageNo: pageNum,
        pageSize: pageSize,
        confirmStatus: _self.status
      }

      Service.claimList('POST', JSON.stringify(params), (function callback(data) {
        // console.log("=====数据：", data)
        if (data.code == 200) {
          ...
          for (var i in listData) {
            if (listData[i].confirmStatus == 0) {
              listData[i].orderStatusDes = "待确认"
            } else if (listData[i].confirmStatus == 1) {
              listData[i].orderStatusDes = "可服务"
            } else if (listData[i].confirmStatus == 2) {
              listData[i].orderStatusDes = "不可服务"
            }
            ...
            listData[i].appointmentTime = fn.formatTime(listData[i].appointmentTime, 'Y-M-D h:m:s')
          }
