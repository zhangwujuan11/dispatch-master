...
var vm = new Vue({
    ...
    methods: {
        ...
        btnUploadFile(item) {
            ...
            reader.onloadstart = (e) => {
                if (item.name == "营业执照") {
                    _self.imgShopZero.isShowMask = true;
                }
                if (item.name == "休息区") {
                    _self.imgShopOne.isShowMask = true;
                }
                if (item.name == "门头") {
                    //_self.imgShopTwo.imgPath = data.data.webPath;
                    _self.imgShopTwo.isShowMask = true;
                }
                if (item.name == "前台区") {
                    //_self.imgShopThree.imgPath = data.data.webPath;
                    _self.imgShopThree.isShowMask = true;
                }
                if (item.name == "施工区") {
                    //_self.imgShopFour.imgPath = data.data.webPath;
                    _self.imgShopFour.isShowMask = true;
                }
                if (item.name == "团队") {
                    //_self.imgShopFive.imgPath = data.data.webPath;
                    _self.imgShopFive.isShowMask = true;
                }
                console.log("开始读取....");
            }
        ...
        getShopDetail() {
            ...
            Service.getShopDetail('GET', params, (function callback(data) {
                ...
                if (data.code == 200) {
					...
					document.getElementById('picker').value=data.data.province + data.data.city + data.data.zone 
        ...
        searchByStationName() {
            var _self = this;
            if (_self.shopInfo.address) {
                map.clearOverlays(); //清空原来的标注  
                var keyword = _self.shopInfo.address;
                //alert(keyword)
                localSearch.setSearchCompleteCallback(function(searchResult) {
                    var poi = searchResult.getPoi(0);
                    _self.shopInfo.lng = poi.point.lng;
                    _self.shopInfo.lat = poi.point.lat;
                });
        ...
        chackInfo() {
            ...
            if (!this.shopInfo.opene || !this.shopInfo.openb) {
                fn.showTip("营业时间不能为空");
                return false;
            }
            if (!this.shopInfo.bankAccount) {
                fn.showTip("银行账号不能为空");
                return false;
            }
            if (!this.shopInfo.bankName) {
                fn.showTip("账户名不能为空");
                return false;
            }
            if (!this.shopInfo.bankUser) {
                fn.showTip("开户行不能为空");
                return false;
            }
		...
		// 省市区
		toCngType(e) {
			document.getElementById("picker").blur();
		},
        //门店更新
        editShop() {
			this.shopInfo.openb = $('#begin_date')[0].value;
			this.shopInfo.opene = $('#end_date')[0].value;
            if (!this.chackInfo()) {
                return false;
            }
            var shopImgList = [];
            if (this.imgShopZero.imgPath) {
                shopImgList.push(this.imgShopZero)
            }
            ...
            var params = {
                ...
                "lat": this.shopInfo.lat,
                "linktel": this.shopInfo.linktel,
                "lng": this.shopInfo.lng,
            ...
            Service.editShop('POST', JSON.stringify(params), (function callback(data) {
                //console.log("数据：", data)
                if (data.code == 200) {
                    fn.showTip("更新成功", "shopInfo.html")
                }
            }))
        }
    ...
    mounted: function() {
		...
		picker.on('picker.valuechange', function (selectedVal, selectedIndex) {
			  let one=selectedIndex[0]
			  let two =selectedIndex[1]
			  let three=selectedIndex[2]
			    _self.shopInfo.province = selectedVal[0];
			    _self.shopInfo.city = selectedVal[1];
			    _self.shopInfo.zone = selectedVal[2];
				 _self.shopInfo.zoneId = city[one].children[two].children[three].value
				 _self.shopInfo.cityId = city[one].children[two].value
				 _self.shopInfo.provinceId = city[one].value
		});
