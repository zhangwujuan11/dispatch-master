...
var vm = new Vue({
  ...
  methods: {
    ...
    btnUploadFile(index) {
      ...
      reader.onloadstart = function (e) {
        _self.memberInfo.surveyCheckList[index].isShowMask = true;
        console.log("开始读取....");
      }
        ...
        if (imgFile.size / 1024 > 1024 * 1.2) {
          fn.dealImage(evt.target.result, {
            ...
            Service.uploadPic('POST', JSON.stringify(params), (function callback(data) {
              ...
              if (data.code == 200) {
                $('.upLoadImgs').eq(index).find(".picFullcar").attr("src", data.data.webPath);
                _self.memberInfo.surveyCheckList[index].isShowMask = false;;
              }
            }))
          });
        } else {
    ...
    add() {
      ...
      var params = {
        damageReasons: this.damageReasons,
        orderAudits: orderAudits,
        userId: this.userId
      }
      console.log("参数请求：", params)
      Service.addSurvey('POST', JSON.stringify(params), (function callback(data) {
        //console.log("数据：", data)
        if (data.code == 200) {
          fn.showTip("提交成功", "surveyList.html?searchType=" + this.searchType)
        }
      }))
  ...
  mounted: function () {
    ...
    Service.queryMember('POST', JSON.stringify(params), (function callback(data) {
      console.log("=====数据：", data)
      if (data.code == 200) {
        for (var i in data.data.surveyCheckList) {
          data.data.surveyCheckList[i].isShowMask = false;
        }
