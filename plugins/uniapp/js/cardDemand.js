...
var vm = new Vue({
    ...
    methods: {
        ...
        getOrderAuditList() {
            var _self = this;
            var params = { rightCardId: tempJson.cardId ? tempJson.cardId : "" };
            Service.getRightCardCheckItems('GET', params, (function callback(data) {
                console.log("=====数据：", data.data)
                if (data.code == 200) {
        ...
        updateMember() {
            ...
            if (!this.userId) {
                ...
                Service.updateMember('POST', JSON.stringify(params), (function callback(data) {
                    if (data.code == 200) {
                        console.log(data.data);
                        _self.userId = data.data;
                        _self.showStepTwo = true;
        ...
        add() {
            ...
            Service.createRightCard('POST', JSON.stringify(params), (function callback(data) {
                if (data.code == 200) {
                    console.log(data.data);
                    fn.showTip("提交成功", "cardOrder.html?orderSn=" + data.data);
                    //window.location.href = "cardOrder.html?orderSn=" + data.data;
                }
            }))
        }
