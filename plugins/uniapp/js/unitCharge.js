...
var vm = new Vue({
    ...
    methods: {
        //添加结算单位
        addUnitCharge(){
            window.location.href="unitChargeAdd.html";
        },

        //查看结算单位详情
        toView(e){
            window.location.href="unitChargeView.html?realId=" + e;
        },

        //编辑结算单位
        edit(e){
            window.location.href="unitChargeAdd.html?realId=" + e;
        },
        ...
        del(e) {
            var realId = e;
            var _self = this;
            fn.commonWindowSur('确定删除此结算单位？')
            $('.commonBtn_suc').on('click', function() {
                var params = { settlementId : realId}
                Service.delShopSettle('GET', params, (function callback(data) {
                if (data.code == 200) {
