    ...
    methods: {
        ...
        searchLevel(index) {
            ...
            if (index == 2) {
                ...
                if(this.startDate){
                    window.location.href = "saleReportDetail.html?startDate=" + this.startDate;
                }
        ...
        //更多销售单
        toMore(e) {
            window.location.href = "saleReportDetail.html?startDate=" + e.reportsDate;
        },
        ...
        getListDataFromNet: function(pageNum, pageSize, successCallback, errorCallback) {
            ...
            Service.getOrderSaleReportForm('POST', JSON.stringify(params), (function callback(data) {
                console.log("=====数据：", data)
                if (data.code == 200) {
                    self.totalPage = data.count;
                    var listData = data.data;
                    for (var i in listData) {
                        for (var j in listData[i].orderSaleVOS) {
                            listData[i].orderSaleVOS[j].saleDate = fn.formatTime(listData[i].orderSaleVOS[j].saleDate, 'Y-M-D')
                        }
                    }
                    //console.log(listData[0].orderSaleVOS[0].saleDate)
