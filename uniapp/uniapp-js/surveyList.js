...
var vm = new Vue({
  ...
  methods: {
    ...
    searchLevel(index) {
      ...
      this.startDate = $('#begin_date')[0].value;
      this.endDate = $('#end_date')[0].value;
    ...
    startOrder(e) {
      ...
      Service.recevieSurvey('POST', JSON.stringify(params), (function callback(data) {
        console.log("=====数据：", data)
        if (data.code == 200) {
    ...
    surRefuse() {
      ...
      if (this.orderStatus == 100) {
        Service.refuseSurvey('POST', JSON.stringify(params), (function callback(data) {
          console.log("=====数据：", data)
          if (data.code == 200) {
            // alert('拒绝')
    ...
    orderRecevie(e) {
      ...
      if (e.userName && e.mobile && e.carType) {
        window.location.href = "surveyDetail.html?orderId=" + e.orderSn + "&searchType=" + this.searchType;
      } else {
        fn.showTip('客户信息不全，请先补全', 'editMember.html?kw=' + e.carNo + '&userId=' + e.userId + '&backUrl=surveyDetail.html?orderId=' + e.orderSn + "&searchType=" + this.searchType);
      }
      // event.preventDefault();
      // event.stopPropagation();
      // var el = event.currentTarget;
      ...
      // window.location.href = "surveyDetail.html?orderId=" + orderSn+"&searchType="+this.searchType;
    },
    //新建勘验单
    add() {
      if (this.searchType == 1) {
        ...
        window.location.href = "orderRepair.html";
      }
    ...
    toDetail(e) {
      if (e.userName && e.mobile && e.carType) {
        if (e.orderStatus != 100 && e.orderStatus != 105) {
          window.location.href = "surveyDetail.html?orderId=" + e.orderSn+"&searchType="+this.searchType;
        } else {
          fn.showTip('请先接单才能查看！');
        }
