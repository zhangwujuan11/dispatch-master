...
var vm = new Vue({
  ...
  methods: {
    ...
    onMaterial(item, index, items, idx) {
         ...
         if (_self.MaterialList.length > 0) {
             ...
             let idx = _self.MaterialList.findIndex(fruit => fruit.text === _self.indexchoose.text)
			
             _self.$delete(_self.MaterialList, idx)
             let images = []
             if (items == 1) {
               imgList.push({
                 imgPath: _self.indexchoose.imgPath,
                 remarks: _self.indexchoose.text
               })
               images.push({
                 imgPath: _self.indexchoose.imgPath,
                 remarks: _self.indexchoose.text
               ...
               imgList.map((obj) => {
                 if (obj.imgId === items.imgId) {
                   obj.imgPath = _self.indexchoose.imgPath;
                   obj.remarks = _self.indexchoose.text
                 }
               })
               images.push({
                 imgId: items.imgId,
  ...
  hideAuthorityMask(){
    ...
    //上传图片
    btnUploadFile(item, index, items, idx) {
      $('.authority_mask').css('display','none');
      var _self = this;
      ...
      reader.onload = async (evt) => {
        ...
        if (imgFile.size / 1024 > 1024 * 1.2) {
          fn.dealImage(evt.target.result, {
            ...
            if (!_self.shopInfo.carg || _self.shopInfo.carg == " ") {
              fn.showTip("请先选择车型");
              return false;
            } else if (!_self.shopInfo.personId) {
              fn.showTip("请选择店员");
              return false;
            } else {
              var params = {
              ...
              Service.uploadPic('POST', JSON.stringify(params), (function callback(data) {
                ...
                if (data.code == 200) {
                  ...
                  if (item.checkItemId == 67 || item.checkItemId == 68 || item.checkItemId == 69 || item.checkItemId == 70 || item.checkItemId == 71) {
                    _self.shopInfo.materialItemList[index].imgList = _self.shopInfo.materialItemList[index].imgList.concat(images)
                  }
                  if (item.checkItemId == 73 || item.checkItemId == 74 || item.checkItemId == 75 || item.checkItemId == 76) {
                    if (idx != undefined) {
                      _self.shopInfo.materialItemList[index].imgList[idx].imgPath = data.data.webPath
                    } else {
                      _self.shopInfo.materialItemList[index].imgList = _self.shopInfo.materialItemList[index].imgList.concat(images)
                    }
                  }
